...
export function poolSeries(pool: string, hours = 48): Promise<SeriesPoint[]> {
  return cached(`pool:${pool}:${hours}`, async () => {
    const body = (await get(`/pools/${pool.toLowerCase()}/ohlcv/hour?limit=${hours}`)) as {
...
export function tokenTopPool(token: string): Promise<string | null> {
  return cached(`token:${token}`, async () => {
